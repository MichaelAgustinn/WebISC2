            ...
            <div class="col-md-12">
                    ...
                    <div class="card-header d-flex flex-wrap justify-content-between align-items-center gap-2">
                        ...
                        <form action="{{ route('creation.index') }}" method="GET"
                            ...
                            @if (Auth::user()->role == 'Admin' || Auth::user()->role == 'Pengurus')
                                <select name="divisi" class="form-select form-select-sm" style="width: 140px;">
                                    <option value="">Semua Divisi</option>
                                    @foreach (['Mobile', 'Website', 'IoT', 'UIUX', 'SistemCerdas'] as $div)
                                        <option value="{{ $div }}"
                                            {{ request('divisi') == $div ? 'selected' : '' }}>
                                            {{ $div }}
                                        </option>
                                    @endforeach
                                </select>
                ...
                @if (method_exists($creations, 'links'))
                    <div class="card-footer d-flex justify-content-center">
                        {!! $creations->links('pagination::bootstrap-5') !!}
                    </div>
                @endif
