...
@section('content')
    <div x-data="formBuilder(@js($form->fields))" class="max-w-4xl mx-auto pb-10">
        ...
        <form action="{{ route('forms.update', $form->id) }}" method="POST" id="dynamicForm">
            @csrf
            @method('PUT')
                    ...
                    <div
                        ...
                        <div class="mt-4 mb-6">
                            <div x-show="field.type === 'text' || field.type === 'number'"
                                class="border-b border-dotted border-gray-300 w-1/2 pb-2 text-gray-400 text-sm">Teks jawaban
                                singkat</div>
                            <div x-show="field.type === 'textarea'"
                                class="border-b border-dotted border-gray-300 w-3/4 pb-2 mt-4 text-gray-400 text-sm">Teks
                                jawaban panjang</div>
            ...
            <div
                ...
                <a href="{{ route('forms.index') }}"
                    class="px-6 py-2.5 rounded-lg text-gray-600 font-medium hover:bg-gray-200 transition">
                    Batal
                </a>
                <button type="submit"
    ...
    <script>
        function formBuilder(initialFields) {
            return {
                // Gunakan initialFields dari controller, jika kosong buat 1 default
                ...
                addField() {
                    this.fields.push({
                        id: Date.now(), // Generate ID besar khusus untuk JS (penanda item baru)
                        label: '',
                        type: 'text',
                        is_required: false
                    });
